        ...
        while len(self.OPEN) > 0 and steps < self.max_steps and (u.i, u.j) != self.goal:
            ...
            for d in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if n not in self.obstacles and n not in self.CLOSED:
                    h = np.sqrt(((n[0] - self.goal[0])**2 + (n[1] - self.goal[1])**2)*steps) # Manhattan distance as a heuristic function
                    heappush(self.OPEN, Node(n, u.g + 1, h))

                    # h = (abs(n[0] - self.goal[0]) + abs(n[1] - self.goal[1]))*steps  # Manhattan distance as a heuristic function
                    # heappush(self.OPEN, Node(n, u.g + 1, h))
